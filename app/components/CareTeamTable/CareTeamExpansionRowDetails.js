    ...
    <div>
      ...
      {participants && participants.length > 0 ?
        participants.map((participant) => {
          ...
          const fullName = memberFirstName.concat(' ').concat(memberLastName);
          return (
            <TableRow key={uniqueId()} columns={column}>
              <TableRowColumn>{fullName}</TableRowColumn>
              <TableRowColumn>{upperFirst(memberType)}</TableRowColumn>
              <TableRowColumn>{upperFirst(roleDisplay)}</TableRowColumn>
